...
namespace NatureLab {
    ...
    {
        glxm::vec2 m_position;
        glxm::vec2 m_velocity;
        glxm::vec2 m_acceleration;
        float m_mass;

        inline ForceV4System() {
            this->m_position = glxm::vec2(450, 350);
            this->m_velocity = glxm::vec2(0.01f, 0.01f);
            this->m_acceleration = glxm::vec2(0.01f, 0.01f);
            this->m_mass = 10.0f;
        }

        inline ForceV4System(float mass, glxm::vec2 position = glxm::vec2(50, 600)) {
    ...
    struct AttractorForceV4 {
        ...
        float m_mass;
        float m_gravity;
        glxm::vec2 m_dragOffset;
        bool m_dragging;
        bool m_rollover;

        AttractorForceV4() {
            this->m_position = glxm::vec2(450, 300);
            this->m_mass = 30.0f;
            this->m_gravity = 1.0f;
        ...
        glxm::vec2 calculateAttraction(const ForceV4System& v) {
            // Calculate direction of force
            glxm::vec2 force = m_position - v.m_position;
            // Distance between objects       
            float distance = glxm::vec2::magnitude(force);
    ...
    public:
        ...
        inline void start() override {
            ...
            this->m_ballTexture = SceneAssets::get_texture("ball");
            this->m_attractorTexture = SceneAssets::get_texture("sun");
            ...
            for (int i = 0; i < m_pItems; i++)
                m_natureSystem.push_back(new ForceV4System((rand() % 25 + 25) / 10.0f,
                    glxm::vec2(20 + i * m_width / m_pItems, 600))
                );
        }
        ...
        inline void show() override {
            INature::show();
            this->update();
            sprite->draw(m_attractorTexture, glxm::vec2(m_attractor->m_position.x, m_attractor->m_position.y), glxm::vec2(200.0f, 200.0f), 0);
            for (const auto& it : m_natureSystem) {
                sprite->draw(m_ballTexture, glxm::vec2(it->m_position.x, it->m_position.y), glxm::vec2(it->m_mass * m_sizeTexture, it->m_mass * m_sizeTexture), 0);
            }
        }
