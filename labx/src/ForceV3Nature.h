...
namespace NatureLab {
    ...
    struct ForceV3System
    {
        ...
        inline ForceV3System(float mass, glxm::vec2 position = glxm::vec2(50, 600)) {
    ...
    struct LiquidForceV3 {
        ...
        glxm::vec2 calculateDrag(const ForceV3System& v) {
            // Magnitude is coefficient * speed squared
            auto speed = glxm::vec2::magnitude(v._velocity);
    ...
    public:
        ...
        inline void start() override {
            ...
            this->_liquidTexture = SceneAssets::get_texture("blue");
            ...
            for (int i = 0; i < _balls; i++)
                _natureSystem.push_back(new ForceV3System((rand() % 25 + 25) / 10.0f ,
                    glxm::vec2(20 + i * _width / _balls, 600))
                );
        }
        ...
        inline void show() override {
            INature::show();
            this->update();
            sprite->draw(_liquidTexture, glxm::vec2(0, 0), glxm::vec2(SceneAssets::SCREEN_WIDTH, (SceneAssets::SCREEN_HEIGHT - SceneAssets::LIMIT_HEIGHT) / 2), 0);
            for (const auto& it : _natureSystem) {
                sprite->draw(_ballTexture, glxm::vec2(it->_position.x, it->_position.y), glxm::vec2(it->_mass * 25.0f, it->_mass * 25.0f), 0);
            }
        }
